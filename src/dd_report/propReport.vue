<template>
  <div>
    <van-tabs swipeable title-active-color="#0089ff" color="#0089ff">
      <van-tab title="本月排行">
      ...
      <van-tab title="本单位统计">
        <van-list @load="loadDeptReport">
          <van-cell
            v-for="(item,index) in deptScoreList"
            :key="index"
            center
          ...
          >
            ...
            <span slot="title">{{item.monthorder === 1 ? 'LY12' : item.month-1}}月份</span>
          </van-cell>
...
export default {
  ...
  methods: {
    monthItemClick: function(item) {
      let monthorder = new Date().getMonth() ;
      this.$router.push({
        path: "/adviceinfo",
        query: { deptid: item.proposal_dept, monthorder: monthorder }
      });
    },
    deptItemClick: function(item) {
      this.$router.push({
        path: "/adviceinfo",
        query: { deptid: window.ddUserInfo.remark, monthorder: item.monthorder }
      });
    },
    loadMonthReport: function() {
      axios
        .get(this.global.ddapi + "proposal/MonthScoreList")
        .then(response => {
    ...
    loadDeptReport: function() {
      axios
        .get(this.global.ddapi + "proposal/DeptScoreList", {
          params: { deptId: window.ddUserInfo.department[0] }
        })
        .then(response => {
          if (response.data) {
            let data = [];
